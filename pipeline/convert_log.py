...
def normalize_submitted_prompt(submitted: str) -> str:
    ...
    if s.endswith(_RUN_PROMPT_SUFFIX):
...
def resolve_suite_match(
    ...
    *,
    ...
    tid = str(item.get("id") or item.get("test_id") or "").strip()
    if tid and not tid.startswith(("entry-", "batch-")):
        for row in index:
            if row.get("id") == tid:
                return row
    ...
    for row in index:
        if _prompt_matches(row.get("prompt", ""), prompt):
            return row
    if position is not None and 0 <= position < len(index):
...
def _convert_single(run_log: dict, suite: dict, suite_path: str) -> dict:
    ...
    for i, entry in enumerate(entries):
        ...
        body = normalize_submitted_prompt(submitted)
        ...
        if matched is None:
            for idx_entry in index:
                if _prompt_matches(idx_entry["prompt"], body):
                    matched = idx_entry
                    break
        if matched is None and i < len(index):
            matched = index[i]
