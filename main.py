...
def _run_export(args) -> None:
    ...
    user_id = os.getenv("GENBOUNTY_USER_ID", "").strip() or args.user_id
    ...
    if not user_id:
        user_id = input("  User ID (MongoDB ObjectId): ").strip()
    if not host or not api_key or not user_id:
        print("[-] Host, API key, and User ID are all required.")
        sys.exit(1)
...
def _menu_export() -> None:
    ...
    if not reports:
        ...
        if not path_input:
            return
        args = SimpleNamespace(report=path_input, host="", api_key="", user_id="", default_level=None)
    else:
        labels = [str(p.relative_to(_root)) for p in reports]
        choice = _pick_numbered("Select pipeline report:", labels)
        if not choice:
            return
        args = SimpleNamespace(report=str(_root / choice), host="", api_key="", user_id="", default_level=None)
    _run_export(args)
...
def main() -> None:
    ...
    exp_p.add_argument("--host", default="", help="AIRTA Systems host (or set GENBOUNTY_HOST).")
    exp_p.add_argument("--api-key", default="", help="AIRTA Systems API key (or set GENBOUNTY_API_KEY).")
    exp_p.add_argument("--user-id", default="", help="User ID (or set GENBOUNTY_USER_ID).")
    exp_p.add_argument(
        "--default-severity",
        "--default-level",
        dest="default_level",
    ...
    exp_p.add_argument(
        ...
        type=int,
        default=None,
        help="Results per POST (default: GENBOUNTY_EXPORT_BATCH_SIZE or 25).",
    )
    exp_p.add_argument(
        "--batch-delay",
        type=float,
